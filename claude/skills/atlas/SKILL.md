...
9. **HiDPI fractional-scaling: `--full` produces an oversized canvas with content pinned to the top-left.** On a display at fractional scaling (e.g. 166.7%, device-scale-factor 1.667), `agent-browser screenshot --full` double-applies the scale: the capture canvas is sized at page dimensions times the DPR, but the page rasterizes at 1x. Result: the real UI lands correctly in the TOP-LEFT corner of an oversized blank canvas (e.g. a 4800x2928 canvas holding 2635x1757 of real content). `agent-browser` is a third-party crate so the binary is not patched. TWO-LAYER FIX: (1) ALWAYS post-process a `--full` capture by trimming to its content bounding box. ImageMagick: `convert in.png -bordercolor white -border 1 -trim +repage out.png` (the 1px white border ensures `-trim` works when content touches an edge; this is a near no-op on a correctly sized capture, but apply it ONLY to `--full` outputs, never blanket to every screenshot, because a legitimately small-content state such as a centered gate modal must not be cropped). Python alternative: `PIL.ImageChops.difference(im, white_bg).convert("L").point(lambda p:255 if p>8 else 0).getbbox()` then `im.crop(bbox)`. (2) Optionally try forcing scale 1 at capture with `agent-browser set viewport <W> <H> 1` before the `--full` shot; this is UNVERIFIED and may stop double-application at source, but the trim is the guarantee. DETECTION: compare the `--full` canvas width to a normal viewport shot of the same page (or the known display logical width); a canvas roughly 1.667x larger is the defect.
