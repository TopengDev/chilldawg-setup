...
async function main() {
  ...
  wa.onMessage((msg: StoredMessage) => {
    ...
    execFile("notify-send", [
      "-a", "WhatsApp",
      "-i", msg.is_from_me ? "mail-send" : "mail-message-new",
      "-t", "5000",
      `${direction} ${displayUser}`,
      content.slice(0, 200),
    ], () => {});
    ...
    // Check if muted
    const isMuted = config.mutedChats.some(m =>
      msg.chat_jid === m || msg.chat_jid.includes(m) || chatLabel.toLowerCase().includes(m.toLowerCase())
    );
    ...
    if (isMuted) return; // Skip sound + channel push for muted chats

    // Play sound
    playNotificationSound();

    // Contact mode + superuser tags
    const contactMode = config.contactModes[sender] || config.contactModes[msg.sender_jid] || config.contactModes[msg.chat_jid] || "";
    const modeTag = contactMode ? ` [${contactMode}]` : "";
    const isSuperuser = config.superuserNumber && (msg.sender_jid.includes(config.superuserNumber) || msg.sender_jid === config.superuserJid);
    const suTag = isSuperuser ? " [SUPERUSER]" : "";
    ...
    const prefix = isGroup ? `[Group] ${sender}` : sender;
    try {
      server.server.notification({
        method: "notifications/claude/channel",
        params: {
          content: `${prefix}${modeTag}${suTag}: ${content}`,
          meta: {
            user: displayUser,
  ...
  server.tool(
    "take_over",
    "Force this session to take over the WhatsApp connection from another session",
    {},
    async () => {
      if (wa.isConnected()) return ok({ message: "Already connected." });
      connectionStatus = "connecting...";
      wa.connect().catch((e: any) => { connectionStatus = `error: ${e.message}`; });
      return ok({ message: "Connecting to WhatsApp. Wait a few seconds then check connection_status." });
    }
  );
  ...
  server.tool(
    "set_contact_mode",
    "Set personality mode for a contact (e.g. CLOSE_FRIEND, FORMAL, PROFESSIONAL)",
    {
      contact: z.string().describe("Contact name or JID"),
      mode: z.string().describe("Personality mode (CLOSE_FRIEND, FORMAL, PROFESSIONAL, or empty to clear)"),
    },
    async ({ contact, mode }) => {
      if (mode) {
        config.contactModes[contact] = mode;
      } else {
        delete config.contactModes[contact];
      }
      saveConfig(config);
      return ok({ contact, mode: mode || "(cleared)", contactModes: config.contactModes });
    }
  );

  server.tool(
    "mute_chat",
    "Mute a contact or group (no notifications unless mentioned)",
    {
      chat: z.string().describe("Contact name, phone, or group JID"),
    },
    async ({ chat }) => {
      if (!config.mutedChats.includes(chat)) {
        config.mutedChats.push(chat);
        saveConfig(config);
      }
      return ok({ muted: true, chat, totalMuted: config.mutedChats.length });
    }
  );

  server.tool(
    "unmute_chat",
    "Unmute a contact or group",
    {
      chat: z.string().describe("Contact name, phone, or group JID"),
    },
    async ({ chat }) => {
      config.mutedChats = config.mutedChats.filter(c => c !== chat);
      saveConfig(config);
      return ok({ unmuted: true, chat, totalMuted: config.mutedChats.length });
    }
  );
  ...
  server.tool(
    ...
    {},
    async () => {
      return ok({ mutedChats: config.mutedChats });
    }
  );
